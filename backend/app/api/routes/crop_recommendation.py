...
class CropRecommendationRequest(BaseModel):
 nitrogen:float=Field(ge=0); phosphorus:float=Field(ge=0); potassium:float=Field(ge=0); temperature:float; humidity:float=Field(ge=0,le=100); ph:float=Field(ge=0,le=14); rainfall:float=Field(ge=0); farm_id:str|None=None
@router.post("")
def recommend_crop(payload:CropRecommendationRequest,db:Session=Depends(get_db),current_user:User=Depends(get_current_user)):
 ...
 if model is None: raise HTTPException(status_code=503,detail="Crop recommendation model is not trained yet.")
 farm=None
 if payload.farm_id:
 ...
 if farm: db.add(CropRecommendation(farm_id=farm.id,crop_name=str(prediction),confidence=confidence,reason="ML crop recommendation")); db.commit()
...
def history(farm_id:str,db:Session=Depends(get_db),current_user:User=Depends(get_current_user)):
 ...
 if not farm: raise HTTPException(status_code=404,detail="Farm not found")
 rows=db.scalars(select(CropRecommendation).where(CropRecommendation.farm_id==farm.id).order_by(CropRecommendation.created_at.desc()).limit(50)).all()
 return [{"id":r.id,"crop":r.crop_name,"confidence":r.confidence,"reason":r.reason,"created_at":r.created_at} for r in rows]
